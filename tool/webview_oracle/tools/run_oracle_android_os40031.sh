#!/usr/bin/env bash
# Re-runs the frozen Legado Android oracle for the handset's *current*
# fingerprint, to produce the golden an Android destination row is compared
# against. `run_oracle_android17.sh` is pinned to OS4.0.0.19 and cannot be
# reused: the comparison refuses an Android destination whose fingerprint differs
# from its golden's.
#
# It is byte for byte the Android 17 script with the fingerprint, the System
# WebView version, the stage directory and the golden directory changed, so the
# procedure, the retry rules, the battery/foreground handling and the accepted
# evidence window are the ones already exercised on this device. The frozen APKs
...
GOLDEN='evidence/android-os4.0.0.31'
...
log "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] ANDROID-OS4.0.0.31-GOLDEN-RUN-BEGIN"
...
console.log('android os4.0.0.31 golden verification: 14/14');
NODE
log "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] ANDROID-OS4.0.0.31-GOLDEN-RUN-COMPLETE acceptedMethods=16"

rm -f "$STAGE/installed-application.apk" "$STAGE/installed-instrumentation.apk"
sed -i 's/\r$//' "$LOG"
mkdir -p "$ROOT/$GOLDEN"
cp "$STAGE"/WV-*.json "$ROOT/$GOLDEN/"
cp "$LOG" "$ROOT/$GOLDEN/run.log"
