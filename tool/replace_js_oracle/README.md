...
| `js-error` | observed | pass | content unchanged and the rule stays enabled on both sides; product notice `replaceRuleFailed(js-error, js: Error: boom…)` (the code and its arguments, the form `compare.dart` records since #72) — the error argument keeps the script's own message since #89, where the committed `comparison.json` holds the earlier run's bare `js`; frozen logs and toasts |
