...
| P3 | **Migration** (Windows) — **partially implemented** | two distinct migrations: (a) Liber's own retired JSON stores (`online_reading.json`, `local_books.json`, `migration_state.json`) into a space, done and driven (#24); (b) a Legado export — source JSON, `books.json`, and the backup ZIP — into a space, with a loss report, idempotent merge, progress that only advances, and local files flagged for relinking | **Exists:** `lib/store/legacy_import.dart`'s `LegacyImport` (a, evidenced in #24) and `LegadoBackupImport.importJson` (b, JSON object only: `lib/store/legacy_import.dart:579`; the migration picker accepts JSON text only, `lib/main.dart:780`). `flutter test test` with `test/legacy_import_test.dart`, `test/legado_backup_import_test.dart`, `test/migration_test.dart`. **P3 work:** the backup ZIP container with path-traversal rejection, the contract's uncovered families below, and a driven import of a real backup copy with the loss report recorded. | Other platforms are P4. No frozen golden: the migration contract is its own specification. The private ZIPs are **future fixtures**, not current test inputs. Cookies, caches, chapter data and book bytes stay excluded by the contract. |
...
evidence to assemble into the named list; P3 is partially implemented (the Liber
store migration exists and was driven; the Legado backup import is JSON-object
only, no ZIP); P4 has not started (no non-Windows platform projects exist); P5
...
([legado-data-migration-contract.md](legado-data-migration-contract.md),
"Required Fixtures and Tests"), and what the currently named tests actually do.
`test/migration_test.dart` verifies drift schema upgrades, so it is not counted
as Legado-import coverage. Missing and partial families are P3's work.

| # | Contract family | Coverage in the named tests | Status |
|---|---|---|---|
| 1 | Source object/array shapes: nested rules, nulls, booleans, large integers, unknown fields, non-ASCII | partial: unknown fields and non-ASCII sources appear; not the complete family | gap |
| 2 | Invalid sources: blank URL/name, empty arrays, null members, malformed values, domain-policy rejection distinct from JSON validation | partial, and divergent: malformed JSON and top-level arrays are rejected, while a source missing its URL is deliberately accepted by name | gap |
| 3 | Full backup ZIP: `bookshelf.json`, `bookSource.json`, `bookGroup.json`, `config.xml`, absent optional files, path-traversal rejection | absent: `LegadoBackupImport.importJson` takes JSON text and the picker accepts JSON only | P3 work |
| 4 | Network and local books preserving custom metadata, order, opaque variables, nonzero progress | partial: the Liber-store import covers metadata/progress shapes; not a Legado backup assertion | gap |
| 5 | `file://` and `content://` cases: no Android path/URI in the contract, unresolved locals keep metadata/progress | absent: the importer retains the Android path in the imported record and only flags relinking | P3 work |
| 6 | All 64 custom group bits including `Long.MIN_VALUE`; synthetic negative group ids excluded | absent | P3 work |
| 7 | Restore conflicts: same `bookUrl`; different `bookUrl` with the same `(name, author)` | absent as a conflict family; repeating the identical input is covered | P3 work |
| 8 | Repeated imports, progress ordering, source-replacement approval, group union, non-destructive metadata merge | partial: repetition and some progress/merge behaviour | gap |
| 9 | Loss reporting for cookies, caches, chapters, downloads and local bytes | partial: one Cookie loss message is asserted | gap |
| 10 | `BookProgress`: missing fields, nullable title, 64-bit millisecond time, deterministic round trips | absent: the synthetic input uses `bookId`/`textOffset` | P3 work |
| 11 | Legacy UI `books.json` detected as lossy, never accepted as a full package | absent | P3 work |
| 12 | Deterministic canonical output including group names and local correlation hashes | absent | P3 work |
...
| Search result fields (`ruleSearch.intro`, `lastChapter`, `wordCount`, `checkKeyWord`); content title rule (`ruleContent.title`); `ruleBookInfo.canReName` | new ticket | non-empty: 59 / 98 / 54 / 45; 2; 2 |
| Book information fields (`ruleBookInfo.wordCount`; `downloadUrls` parsing) | new ticket + #14 | non-empty: 52 / 3 |
| Login flow (`loginUrl`, `loginUi`, `loginCheckJs`) | #13 | `loginUrl` 32 non-empty / 79 present (47 empty); `loginUi` 5 / 17; `loginCheckJs` 9 / 20 |
| TOC formatting and markers (`updateTime`, `isVolume`, `isVip`, `isPay`) | #13 | non-empty: 12 / 9 / 3 / 1 (`preUpdateJs`/`formatJs` are empty in every source that has them — deferrals) |
| `bookUrlPattern` | #13 | 36 non-empty / 82 present (46 empty) |
| Source variables (`getVariable`/`setVariable`; the `variable` field is unused) | #13 | `getVariable` 5 records (9 occurrences), `setVariable` 3 (5) |
...
| `charset` in request options; non-UTF-8 response decoding | new ticket | 42 records contain the `charset` token (a token observation — the option is written inside the request rule and is not parsed; `escape` appears in none) |
| Per-source concurrency and `concurrentRate` | new ticket | `concurrentRate` 2 non-empty / 15 present; `java.ajax(` 39 records (61 occurrences) |
| `enabledCookieJar` parity (storage is ADR 0011 §3; the flag's semantics are not reproduced) | new ticket | 76 records declare `true` (boolean) |
| `java` multi-URL `ajax`/`ajaxAll`, header-string `connect`, `source.getHeaderMap` | new ticket | 1 record each (token) |
| `book` binding (always `null` today) and chapter variables | new ticket | — (no source-visible count) |
| `toNumChapter` Chinese numerals | new ticket | 3 records (token) |
| JSONPath filters and slices | new ticket | not counted separately |
...
|---|---|---|
| jsoup `:matchText` (a DOM-mutating selector returning pseudo text elements; rejected by name today) | 0 / 0 | new ticket, proposed by this decision |
