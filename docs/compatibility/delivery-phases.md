...
| Linux | exists (`flutter build linux --debug`, CI; the artifact row is uploaded) | exists (CI job runs the shared gate list) | `not-run` — never executed on this platform (the harness is platform-neutral) | #2, WebKitGTK `not-run` | real Linux, or WSL with its distribution recorded |
| macOS | exists (`flutter build macos --debug`, CI; the artifact row is uploaded) | exists (CI job runs the shared gate list) | `not-run` — never executed on this platform | #2, WKWebView `not-run` | the operator's Mac |
