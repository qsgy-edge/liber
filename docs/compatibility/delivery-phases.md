...
| P4 | **Five-platform product shell** | Android, iOS, macOS and Linux project scaffolding, native glue, an app that builds and launches, the per-platform gate rows, and the WebView adapters (#2) | **Tier A — landed (#68, batch 13):** the four platform projects exist (`flutter create` at `com.liber.liber`), each platform's CI job builds its app (`flutter build windows/linux/macos --debug`, `flutter build apk --debug`, `flutter build ios --simulator --no-codesign`) and records the artifact it produced (path + sha256) in `.ci-results/`, and the limits harness runs on the Linux and macOS runners, so those platforms' limits rows **executed for the first time in run `35916986796` and reproduced the Windows shape** (`status: pass` on both; heap limit enforced, the deadline interrupts interpreter work and is still explicitly **not** a hard bound — heavy-loop-body overshoot 380 ms Linux / 679 ms macOS, the OOM-retry shape 7.7 s / 6.7 s, one long native call uninterrupted — matching ADR 0011's accepted residual; the per-run rows are uploaded as `evidence-linux`/`evidence-macos`, while the committed `tool/runtime_limits_prototype/evidence/` stays the Windows record). The Android build was reproduced locally (304 427 927 B debug APK, `libfjs.so` for arm64-v8a/armeabi-v7a/x86_64, `INTERNET` and the baseline's `network_security_config` in the merged manifest); Android is pinned to AGP 8.13.1 + Gradle 8.14 because the plugin set is not AGP-9 ready, and the shared gate list still runs on Windows, Linux and macOS through CI (e.g. run `35053007809` for the Linux/macOS gate rows, per ADR 0009's provenance). **Tier B (machine-dependent, still open):** a launch smoke and, where a machine exists, a driven run — macOS on the operator's Mac, Linux on a real machine or WSL (recording WSL and its distribution when used), Android on a device or AVD, iOS in the Mac's simulator. | No device-side runtime row set exists for Android/iOS (the Android/iOS jobs cross-build the native library and now build the app, but no device is attached), so a mobile platform cannot claim compatibility from build rows alone: ADR 0009 also requires that platform's limits rows and #2 WebView rows. Linux rendered-document path stays `not-run` until #2 proves WebKitGTK. Android/iOS limits rows need their own binding and a device. An app build is a shell row: it is never a compatibility claim (ADR 0002/0009). |
| P5 | **v1-scope Book Source compatibility** | the v1-scope capability rows closed with fixtures and per-platform comparisons, with the named deferrals and accepted divergences kept visible | For every in-v1 row: a differential fixture and a passing comparison on every **claimed** platform; the aggregate stays honest — no coverage gap inside the v1 scope, every `not-run` row named, every `policy-rejected` row named, every accepted divergence named; the operator's used-source set is re-triaged with `dart run tool/source_triage.dart <fjs.dll> <exported sources>` as a supplemental observation (never a gate). | Everything in the deferral table below, with its owner; the frozen goldens of #15/#23/#38 are recorded on the handset, so what remains of those rows is their Android *destination* half, which waits on P4 (the product has no Android application). **P5 does not deliver the differential contract's "frozen-baseline complete" verdict:** that verdict needs no matrix gap, every required fixture on all five platforms, and no baseline-supported source policy-rejected (`book-source-differential-contract.md`, Verdict Aggregation); P5's claim is narrower and must be stated as v1-scope compatibility on named platforms. Batch 14 ran that audit ([`p5-windows-audit.md`](p5-windows-audit.md)): every v1 differential fixture that exists passes on Windows, and 16 in-v1 rows are declared coverage gaps, so no v1-scope claim is recorded yet — the audit lists each gap with its owner. |
| P6 | **Product completion** (v1's last slice) — **landed in batches 13–14** | multi-source search, the frozen precise-search flow, and switch-source with progress remapping; the settings field set lands here too (#27, #28) | **Landed and driven:** multi-source 精确搜索 + 换源 (#40: the frozen formatted name+author filter, the first-exact early stop, the frozen no-match answer, serial per-source search under the rate limiter, the position mapped through the frozen `BookHelp.getDurChapter`; driven on a scratch installation — the exact match switched the book, `text_offset 42` survived onto the new TOC's chapter 3, one shelf row, same book id); #27 (locale-driven conversion with a manual and per-book override, both readers re-rendered without reopening, `java.t2s` still characters only — measured at 5–11 ms release for a 50 000–100 000-code-unit chapter and kept on the UI isolate; driven: the TOC title and the body switched to 繁體（台灣） without reopening); #28 (independent interface/content settings that apply immediately and survive a restart — driven: English interface from the app bar, persisted as `settings` `('', 'interface.language', 'english')` and read back after a process restart, with the book content still Chinese; every ARB file carries the template's key set, and the zh-Hant interfaces are generated from the Simplified template by `liber_text`); and reader images (#67: extraction from the content rules in both shapes, the fetch through the source's own dispatcher incl. the TLS confirmation, the four `imageStyle` layouts, the five-field progress record unchanged); and batch 14's two: the automatic switch-source entry (#69: the frozen `AppConfig.autoChangeSource` trigger on a shelf row whose source was deleted, its own settings page, the frozen candidate order, the content check before a switch is accepted, `isCancelled` threading and a driven run — the switched row kept its id and its mapped position, `text_offset 42` on the shelf and the mapped chapter 第三章 in the reader, one shelf row, re-fetched TOC) and the store layer's copy (#72: a `StoreMessage` seam whose 52 codes render through `AppLocalizations`, the migration loss report and its five-argument summary, the reader's replace-rule notices, the shelf's run status, and the `legacy_import.v1` marker read in both shapes; driven — the English Migration page showed the English summary and the English loss line from a marker the app itself wrote). | Nothing frozen: these flows are product work over search/information/TOC, recorded as observations rather than compatibility rows. Still open and named: the frozen-device layout rows for the `imageStyle` shapes and the `<img>`-list corpus rows (#67/#66 — the handset was not attached during batch 14), the local reader's own notices (Chinese under every interface — the follow-up ticket batch 14 opened). |
...
two residuals named in its row (#46); P4's Tier A has landed (#68: the four
platform projects, one app build per platform in CI, the limits harness wired on
Linux and macOS) while its Tier B rows stay machine-dependent; P5
has capability slices landed and, after batch 14's audit
([`p5-windows-audit.md`](p5-windows-audit.md)), **no v1-scope claim**: every Windows differential
fixture that exists passes, but 16 in-v1 rows have no fixture at all and one row's only evidence is
a report no command re-derives — the audit records each row with its owner; P6's flows have landed and
been driven (batches 13–14), with the named gaps above.
...
| TOC formatting and markers (`ruleToc.updateTime`, `isVolume`, `isVip`, `isPay`) | **landed (#13)** — extracted, persisted (store v6) and shown; `preUpdateJs`/`formatJs` stay #13 | non-empty: 12 / 9 / 3 / 1 (`preUpdateJs`/`formatJs` are empty in every source that has them — deferrals) |
...
| Local `jsLib` shared scope; remote `jsLib` stays with the ticket | local **landed (#13)**; remote `jsLib` stays in #13 | 3 non-empty |
...
| Multi-URL page results and image styling (`imageStyle`) | multi-URL **landed (#14)** (both rule shapes, declared order, concurrency 1); `imageStyle` and the image half stay #14 | `imageStyle` 28 non-empty / 35 present; multi-URL results not independently counted |
| WebView request path (`webView`, `webViewDelayTime`) | **landed on Windows and Android (#55)** — the Windows destination rows (re-collected on the #75 revision in batch 15) and the Android oracle/adapter sweep; the five-platform aggregate and Linux/macOS/iOS stay `not-run` (#56) | 18 records mention `webView` (token); `ruleContent.webJs` is present in 12 and empty in all of them |
| User-confirmed browser and captcha hatches (`startBrowser*`, `getVerificationCode`, `openUrl`) | **landed (#32)** — the members show the user-confirmed surface (visible page or image dialog, five-minute cap, the script's deadline paused) | 3 / 2 |
| TLS per-source exception on certificate failure | #30 (#75 for the WebView halves) | (policy row: validation default, one confirmation per source and host, the stored exception read by the transport; since #75 both WebView paths reach the same confirmation and the same store row — the engine-level row stays `not-run` because WebView2 cannot surface a certificate error and Android waits on the handset) |
...
| Linux | exists (`flutter build linux --debug`, CI; the artifact row is uploaded) | exists (CI job runs the shared gate list) | **executed** in CI run `35916986796` (`status: pass`, uploaded as `evidence-linux`; the probes' POSIX branch compiled and reproduced on the runner) | #2, WebKitGTK `not-run` | real Linux, or WSL with its distribution recorded |
| macOS | exists (`flutter build macos --debug`, CI; the artifact row is uploaded) | exists (CI job runs the shared gate list) | **executed** in CI run `35916986796` (`status: pass`, uploaded as `evidence-macos`, on the runner's arm64) | #2, WKWebView `not-run` | the operator's Mac |
| Android | exists (`flutter build apk --debug`, CI, and reproduced locally; AGP 8.13.1 + Gradle 8.14) | exists on the device for the WebView adapter corpus (see the WebView column) | `not-run` | **executed on the handset 2026-09-24** (`evidence/android-os4.0.0.31/` golden 14/14 and `evidence/android-destination/` 14 rows through the product adapter — 13 `match`, `WV-14` `policy-rejected`), both on System WebView 155.0.8059.4; the archived prototype rows were refused by name and are superseded | device or AVD |
| iOS | exists (`flutter build ios --simulator --no-codesign`, CI; not executable from this host) | no device-side row set today | `not-run` | #2, WKWebView `not-run` | simulator on the Mac |

Three desktop platforms have runtime gate rows today (Windows, Linux, macOS);
Windows is the only platform whose **application has been driven** (a launch
smoke and a driven run exist for it, and it is the only integrated application
the operator runs). The other four build their app in CI — a shell row. Until a
platform's four row families exist, claims stay narrower and say so — build and
launch rows alone are a shell claim, not a compatibility claim.
