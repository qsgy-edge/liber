...
| Android | exists (`flutter build apk --debug`, CI, and reproduced locally; AGP 8.13.1 + Gradle 8.14) | no device-side row set today | `not-run` | archived pre-split evidence: Android WebView adapter, 14 rows (13 pass, 1 policy-rejected) in `liber-archive`; #2 owns re-establishing it, and no current-product claim follows | device or AVD |
