...
|---|---|---|
| Login flow (`loginUrl`, `loginUi`, `loginCheckJs`) | #59/#60 | product tests + a driven run (batch 11) |
...
| `book`/`chapter` read-only snapshots and the `*Variable` refusals | #43 | host gate; the `*Variable` members are recorded as "no owner, a re-triage candidate" |
...
path with no frozen stage; libfjs's runtime tests (#35) are CI gates, not compatibility rows; and the
`*Variable` row needs a *decision* about whether it stays in v1 before it needs a fixture.

## 4. What the next batch should do with this

1. Pick the gap families whose fixture is cheapest per unit of claim (the request/host-surface ones
   have device harnesses already: `tool/nested_oracle/RequestOracle.java` and `tool/html_oracle/`)
   and build them, one ticket each or one umbrella ticket.
2. The device-backed rows (#66's list read, #67's `imageStyle` layouts) need the handset attached;
   `adb devices` was empty at this audit, so they stay `not-run`.
