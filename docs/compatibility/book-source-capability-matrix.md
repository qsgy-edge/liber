...
| Request body media type and framing | okhttp-4.12.0 `RequestBody$Companion.create(String, MediaType)`, `AnalyzeUrl.kt:381-393`, `OkHttpUtils.kt:113-115` | 🟡 device-pinned by REQUEST-01 (`redirect-300-post` … `redirect-308-post`): a declared media type that names no charset reaches the wire as `<declared>; charset=utf-8`, a declared charset is kept as written, and the body carries its `Content-Length` instead of a chunked stream — the product reproduces all three (`lib/source/http_source_transport.dart`). One half stays unreproduced: the frozen call also writes the body's *bytes* with that resolved charset, while this transport writes UTF-8; no fixture in the corpus declares a non-UTF-8 body charset |
