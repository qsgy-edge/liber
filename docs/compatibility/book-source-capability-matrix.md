...
| `ruleContent` | `ContentRule.kt:13-21` | ✅ `content`/`title`/`nextContentUrl`; 🟡 `replaceRegex` (`##regex##replacement` and `{{chapter.title}}` only; a source that also writes an inline `##` replacement in `content` is ⛔ rejected with an explicit error instead of being applied twice — the frozen content-stage replacement belongs to #17); ❌ `webJs`/`sourceRegex`/`imageStyle`/`imageDecode`/`payAction` (the content stage's `webView` **URL option** is wired since #55; the rule's own `webJs`/`sourceRegex` fields still are not) |
...
| WebView request path | `BackstageWebView`, `webView*` options | 🟡 executed headlessly through `lib/source/book_source_webview_adapter.dart` (#55, ADR 0011 §4): the Windows destination sweep compares 13 `match` and WV-14 `policy-rejected`, while the Android destination rows and the five-platform aggregate are `not-run` |
