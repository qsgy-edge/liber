...
| `java.get`/`put` rule state | 🟡 persistent per-source variables remain owned and capped as ADR 0011 §3/#21/#37 require; `java.get("bookName")`/`java.get("title")` read actual bound stage values when supplied. Chapter `title`/`url` snapshots exist, but book/chapter variable members refuse by name (#43). Frozen chapter/book variable routing is not implemented; `source.getVariable`/`setVariable` remains #13. |
