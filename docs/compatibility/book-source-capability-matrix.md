...
| `java.get`/`put` rule state | 🟡 persistent per-source variables remain owned and capped as ADR 0011 §3/#21/#37 require; `java.get("bookName")`/`java.get("title")` read that same persistent state without snapshot shadowing. Chapter `title`/`url` snapshots exist, but book/chapter variable members refuse by name (#43). Frozen chapter/book variable routing is not implemented; the per-source `source.getVariable`/`setVariable` variable landed in #59 on the same `sourceVariable_<sourceKey>` entry. |
...
| `java` WebView family (`webView*`, `startBrowser*`, `getVerificationCode`, `getWebViewUA`) | 🟡 `webView`/`webViewGetSource`/`webViewGetOverrideUrl` are wired through `BookSourceWebViewAdapter` since #55; `startBrowser*`/`getVerificationCode`/`openUrl` refuse by name until #32; ✅ `getWebViewUA` is emulated (#31) — it answers the product's non-empty, platform-plausible WebView user agent. The family is not ✅: the adapter behaviour has executed Windows rows, but the `java.webView*` members themselves are only unit-tested (the WV corpus drives the adapter, not the host members) and the Android destination rows are `not-run` |
...
| Login flow and login headers | `BaseSource.kt:134-196`, `ui/login/SourceLoginDialog.kt` | 🟡 the `loginCheckJs` stage hook is #59: the script runs once per stage on its first response (`WebBook.kt:71,121,178,253,336` — the frozen `nextTocUrl`/`nextContentUrl` pages never run it either), sees the response as `result`, and the response it returns is what the rest of the stage reads; a script that throws or returns a non-response fails the stage, with the script's own error recorded in the source log and shown as a source notice, and the stage request surface behind it (`getResponse`, `getStrResponse`, `initUrl`) belongs to the hook. Since #60 `login()`, the `loginUi` form, the login headers and `loginInfo` are implemented: the stored login header is part of the source's header map and every request of that source carries it (`AnalyzeUrl.kt:89,124`). Two divergences are recorded — the login header goes *under* the request's own headers, so a key the source's static `header` rule also declares keeps the static value here where the frozen login header wins, and the jar's `Cookie` is only added when the request brought none, where the frozen `setCookie` merges the two; and a `loginUrl` script's `java.startBrowser*`/`getVerificationCode`/`openUrl` calls keep the named #32 refusal rather than opening a confirmation |
