...
| `loginUrl`, `loginUi`, `loginCheckJs` | `BaseSource.kt:134-182`, `WebBook.kt:71,121,178,253,336` | 🟡 `loginUrl` and `loginCheckJs` are carried: a source that declares either opens, and the check script runs after each stage's first response (#59). ⛔ the interactive login (`login()`, `loginUi`, login headers, `loginInfo`) stays with #60; the fields are read but pressing 登录 does nothing |
...
| Login flow and login headers | `BaseSource.kt:134-182` | 🟡 the `loginCheckJs` stage hook is #59: the script runs once per stage on its first response (`WebBook.kt:71,121,178,253,336` — the frozen `nextTocUrl`/`nextContentUrl` pages never run it either), sees the response as `result`, and the response it returns is what the rest of the stage reads; a script that throws or returns a non-response fails the stage, with the script's own error recorded in the source log and shown as a source notice, and the stage request surface behind it (`getResponse`, `getStrResponse`, `initUrl`) belongs to the hook. ❌ `login()`, `loginUi`, the login headers and `loginInfo` are #60 |
