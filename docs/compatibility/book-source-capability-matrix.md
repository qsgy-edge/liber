...
| Binding `java` | 🟡 `connect`/`ajax`/`get`/`head`/`post`, ordered `ajaxAll` response accessors, header-string `connect`, and argument-count `get` overloads. A list passed to `ajax` selects its first URL, as frozen `JsExtensions.kt:91-96` does. #43 is source-derived and Windows-tested, not a frozen-device result; [limits and evidence](host-surface-43.md) |
| Binding `source` | 🟡 `getKey`, `getName`, `getTag`, `getVariable`, `put`/`get`, common fields and `getHeaderMap()`/`getHeaderMap(false)` (static or script header, default UA, fresh map). `getHeaderMap(true)` refuses by name with #13 because login headers are deferred; no login parity claim. [Evidence](host-surface-43.md) |
