...
| `java.get('bookName')` / `java.get('title')` | `AnalyzeUrl.kt:371-379` | Read actual bound stage values when present. Other `java.get/put` calls keep ADR 0011 §3's source-owned persistent state, as the controller explicitly confirmed for this slice. Frozen chapter/book variable routing remains unimplemented. `source.get/put` continue to address source state. |
